...
def ai_attack():
    """
        Recherche des opportunités de gagner et bloquer le joueur si nécessaire en simulant des coups.
    """
    #regarde si elle peut gagner 
    for i in range(3): 
        for j in range(3):
            if np_board[i, j] == " ":
...
def ai_easy():
    ...
    if len(empty_cellule) > 0:
        ...
        draw()
        messagebox.showinfo("Draw", "égalité")
        reset()
